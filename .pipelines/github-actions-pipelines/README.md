...
Reference copy of this component's GitHub Actions deployment tooling, sourced from `dpn-containerised-deployment-service` (branch `feature/merge-azure-aws`) on 2026-09-22.
...
- `workflows/dpn-gha-keycloak-cd.yaml` — the install/deploy workflow, handles all three clouds via its own `cloud` input (not three separate files)

The Helm values for this component's chart on each cloud now live where they belong — alongside the chart itself, under `charts/keycloak/values/<cloud>/<environment>-<cluster>.yaml` (e.g. `values/aws/dev-dpn01.yaml`) — not in this folder. This mirrors the `values/{aws,azure,gcp}/<environment>-<cluster>.yaml` folder structure used in the source repo `dpn-containerised-deployment-service` (GCP excluded here). It sits alongside, and does not touch, the existing flat `values-<environment>-<cluster>.yaml` files used by the Azure DevOps pipelines.
